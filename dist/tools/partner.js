...
export async function listPartnerSubscriptions(params) {
    ...
    return client.list('/partner/billing/subscriptions', {
        version: '2.0',
        params: {
...
export async function getPartnerSubscription(subscriptionId) {
    const client = getClient();
    return client.getOne(`/partner/billing/subscriptions/${subscriptionId}`, { version: '2.0' });
}
export async function createSubscriptionToken(data) {
    const client = getClient();
    return client.post('/partner/billing/token', data, { version: '2.0' });
}
export async function getSubscriptionByToken(token) {
    const client = getClient();
    return client.getOne(`/partner/billing/token/${token}`, { version: '2.0' });
}
export async function createUpdateSubscriptionToken(data) {
    const client = getClient();
    return client.post('/partner/billing/token/update', data, { version: '2.0' });
}
