...
export async function search(params) {
    ...
    // Product attributes
    if (params.sku)
        searchParams.sku = params.sku.toLowerCase();
    if (params.supplier_id)
        searchParams.supplier_id = params.supplier_id;
    if (params.brand_id)
        searchParams.brand_id = params.brand_id;
    if (params.tag_id)
        searchParams.tag_id = params.tag_id;
    if (params.product_type_id)
        searchParams.product_type_id = params.product_type_id;
    if (params.variant_parent_id)
        searchParams.variant_parent_id = params.variant_parent_id;
    // Customer attributes
    if (params.customer_code)
        searchParams.customer_code = params.customer_code;
    if (params.first_name)
        searchParams.first_name = params.first_name;
    if (params.last_name)
        searchParams.last_name = params.last_name;
    if (params.company_name)
        searchParams.company_name = params.company_name;
    if (params.mobile)
        searchParams.mobile = params.mobile;
    if (params.phone)
    ...
    if (params.invoice_number)
        searchParams.invoice_number = params.invoice_number;
    if (params.customer_id)
        searchParams.customer_id = params.customer_id;
    if (params.user_id)
        searchParams.user_id = params.user_id;
    if (params.outlet_id)
        searchParams.outlet_id = params.outlet_id;
