...
export async function listVariantAttributes(params) {
    ...
    return client.list('/variant_attributes', {
        version: '2.0',
        params: {
...
export async function getVariantAttribute(attributeId) {
    const client = getClient();
    return client.getOne(`/variant_attributes/${attributeId}`, { version: '2.0' });
}
export async function createVariantAttribute(data) {
    const client = getClient();
    return client.post('/variant_attributes', data, { version: '2.0' });
}
export async function updateVariantAttribute(attributeId, data) {
    const client = getClient();
    return client.put(`/variant_attributes/${attributeId}`, data, { version: '2.0' });
}
export async function deleteVariantAttribute(attributeId) {
    const client = getClient();
...
export async function addProductVariant(productId, data) {
    const client = getClient();
    return client.post(`/products/${productId}/variants`, data, { version: '2.1' });
}
