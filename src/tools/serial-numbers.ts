...
export async function listSerialNumbers(params: {
  ...
  return client.list<SerialNumber>('/serialnumbers', {
    version: '2.0',
    params: {
...
export async function getSerialNumber(serialNumberId: string): Promise<SingleResponse<SerialNumber>> {
  const client = getClient();
  return client.getOne<SerialNumber>(`/serial-numbers/${serialNumberId}`, { version: '2.0' });
}

export async function createSerialNumber(data: SerialNumberCreateInput): Promise<SingleResponse<SerialNumber>> {
  const client = getClient();
  return client.post<SingleResponse<SerialNumber>>('/serialnumbers', data, { version: '2.0' });
}

export async function deleteSerialNumber(serialNumberId: string): Promise<void> {
  const client = getClient();
