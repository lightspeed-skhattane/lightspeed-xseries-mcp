...
import type {
  ...
  Consignment,
  ConsignmentCreateInput,
  ConsignmentProduct,
  ConsignmentProductCreateInput,
  PaginatedResponse,
  SingleResponse,
...
export async function listInventory(params: {
  ...
  return client.list<Inventory>('/inventory', {
    version: '2.0',
    params: {
...
export async function getInventory(inventoryId: string): Promise<SingleResponse<Inventory>> {
  const client = getClient();
  // Note: There is no /inventory/{id} endpoint in the API.
  // We fetch from the list and filter by ID instead.
  const response = await client.list<Inventory>('/inventory', {
    version: '2.0',
    params: {},
  });
  ...
  const inventory = inventoryData.find((inv: Inventory) => inv.id === inventoryId);
...
export async function updateInventory(inventoryId: string, data: Partial<InventoryUpdateInput>): Promise<SingleResponse<Inventory>> {
  const client = getClient();
  return client.put<SingleResponse<Inventory>>(`/inventory/${inventoryId}`, data, { version: '2.0' });
}
...
export async function listConsignments(params: {
  ...
  const client = getClient();
  return client.list<Consignment>('/consignments', {
    version: '2.0',
    params: {
      page_size: params.page_size,
      after: params.after,
...
export async function getConsignment(consignmentId: string): Promise<SingleResponse<Consignment>> {
  const client = getClient();
  return client.getOne<Consignment>(`/consignments/${consignmentId}`, { version: '2.0' });
}

export async function createConsignment(data: ConsignmentCreateInput): Promise<SingleResponse<Consignment>> {
  const client = getClient();
  return client.post<SingleResponse<Consignment>>('/consignments', data, { version: '2.0' });
}

export async function updateConsignment(consignmentId: string, data: Partial<ConsignmentCreateInput>): Promise<SingleResponse<Consignment>> {
  const client = getClient();
  return client.put<SingleResponse<Consignment>>(`/consignments/${consignmentId}`, data, { version: '2.0' });
}
...
export async function listConsignmentProducts(consignmentId: string, params: {
  page_size?: number;
  after?: string;
}): Promise<PaginatedResponse<ConsignmentProduct>> {
  const client = getClient();
  return client.list<ConsignmentProduct>(`/consignments/${consignmentId}/products`, {
    version: '2.0',
    params: {
      page_size: params.page_size,
      after: params.after,
...
export async function addConsignmentProduct(consignmentId: string, data: ConsignmentProductCreateInput): Promise<SingleResponse<ConsignmentProduct>> {
  const client = getClient();
  return client.post<SingleResponse<ConsignmentProduct>>(`/consignments/${consignmentId}/products`, data, { version: '2.0' });
}

export async function updateConsignmentProduct(
  consignmentId: string,
  productId: string,
  data: Partial<ConsignmentProductCreateInput>
): Promise<SingleResponse<ConsignmentProduct>> {
  const client = getClient();
  return client.put<SingleResponse<ConsignmentProduct>>(`/consignments/${consignmentId}/products/${productId}`, data, { version: '2.0' });
}

export async function deleteConsignmentProduct(consignmentId: string, productId: string): Promise<void> {
  const client = getClient();
  await client.delete(`/consignments/${consignmentId}/products/${productId}`, { version: '2.0' });
}

export async function getConsignmentTotals(consignmentId: string): Promise<SingleResponse<unknown>> {
  const client = getClient();
  return client.getOne<unknown>(`/consignments/${consignmentId}/totals`, { version: '2.0' });
}

export async function bulkUpdateConsignmentProducts(
  consignmentId: string,
  data: { products: ConsignmentProductCreateInput[] }
): Promise<PaginatedResponse<ConsignmentProduct>> {
  const client = getClient();
  return client.put<PaginatedResponse<ConsignmentProduct>>(`/consignments/${consignmentId}/products`, data, { version: '2.0' });
}
