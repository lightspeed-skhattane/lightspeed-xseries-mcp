...
export async function listQuotes(params: {
  ...
  return client.list<Quote>('/quotes', {
    version: '2.0',
    params: {
...
export async function getQuote(quoteId: string): Promise<SingleResponse<Quote>> {
  const client = getClient();
  return client.getOne<Quote>(`/quotes/${quoteId}`, { version: '2.0' });
}
...
export async function createQuote(data: {
  ...
  note?: string;
  expiry_date?: string;
  line_items?: { product_id: string; quantity: number; price?: number }[];
}): Promise<SingleResponse<Quote>> {
  const client = getClient();
  return client.post<SingleResponse<Quote>>('/quotes', data, { version: '2.0' });
}
...
export async function updateQuote(quoteId: string, data: {
  ...
  const client = getClient();
  return client.put<SingleResponse<Quote>>(`/quotes/${quoteId}`, data, { version: '2.0' });
}
...
export async function convertQuoteToSale(quoteId: string): Promise<SingleResponse<unknown>> {
  const client = getClient();
  return client.post<SingleResponse<unknown>>(`/quotes/${quoteId}/convert`, {}, { version: '2.0' });
}
