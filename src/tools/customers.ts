...
export async function listCustomers(params: {
  ...
  return client.list<Customer>('/customers', {
    version: '0.9',
    params: {
...
export async function getCustomer(customerId: string): Promise<SingleResponse<Customer>> {
  const client = getClient();
  return client.getOne<Customer>(`/customers/${customerId}`, { version: '2.0' });
}

export async function createCustomer(data: CustomerCreateInput): Promise<SingleResponse<Customer>> {
  const client = getClient();
  return client.post<SingleResponse<Customer>>('/customers', data, { version: '0.9' });
}

export async function updateCustomer(customerId: string, data: Partial<CustomerCreateInput>): Promise<SingleResponse<Customer>> {
  const client = getClient();
  return client.put<SingleResponse<Customer>>(`/customers/${customerId}`, data, { version: '2.0' });
}
...
export async function listCustomerGroups(params: {
  ...
  const client = getClient();
  return client.list<CustomerGroup>('/customer_groups', {
    version: '2.0',
    params: {
      page_size: params.page_size,
      after: params.after,
...
export async function getCustomerGroup(groupId: string): Promise<SingleResponse<CustomerGroup>> {
  const client = getClient();
  return client.getOne<CustomerGroup>(`/customer_groups/${groupId}`, { version: '2.0' });
}

export async function createCustomerGroup(data: CustomerGroupCreateInput): Promise<SingleResponse<CustomerGroup>> {
  const client = getClient();
  return client.post<SingleResponse<CustomerGroup>>('/customer_groups', data, { version: '2.0' });
}

export async function updateCustomerGroup(groupId: string, data: Partial<CustomerGroupCreateInput>): Promise<SingleResponse<CustomerGroup>> {
  const client = getClient();
  return client.put<SingleResponse<CustomerGroup>>(`/customer_groups/${groupId}`, data, { version: '2.0' });
}
...
export async function getGroupCustomers(groupId: string, params: {
  page_size?: number;
  after?: string;
}): Promise<PaginatedResponse<Customer>> {
  const client = getClient();
  return client.list<Customer>(`/customer_groups/${groupId}/customers`, {
    version: '2.0',
    params: {
      page_size: params.page_size,
      after: params.after,
...
export async function bulkUpdateCustomerTaxes(data: { customer_ids: string[]; tax_id: string }): Promise<void> {
  const client = getClient();
